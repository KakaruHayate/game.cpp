...
本地实测（10s 片段，seed 42；RTX 2070 Vulkan vs CPU AVX2；各组合音符输出一致）：

| EP | 权重 | nsteps=1 | nsteps=8 |
|----|------|---------|---------|
| CPU | Q8_0 | 5.9s | 14.0s（DBCache 开启） |
| Vulkan（暖态） | F32 | **0.39s** | **0.65s** |
| Vulkan（暖态） | Q8_0 | 0.42s | 1.09s |

- GPU 后端暖态比 CPU **快 10–20×**：有独立显卡时优先用 `-full`（F32）包；内存受限可用 Q8，差距很小。
...
|---|---|---|---|
| [ggml](https://github.com/ggerganov/ggml) | `v0.19.0` tag（暂时锚定 — 见 [AGENT.md](AGENT.md) "ggml version gate"） | MIT | MIT |
